...
| Official task statement | Exact modules and lessons | Guided labs, incident drills, and projects | Gates |
|---|---|---|---|
| **6.1 Implement techniques for identity and access management at scale.** | [AWS](../12-aws/README.md): [Accounts, IAM, and the AWS API](../12-aws/01-identity-and-api.md); [Security](../20-security/README.md): [Identity, authentication, and authorization](../20-security/02-identity-and-access.md), [Secrets and credential lifecycles](../20-security/04-secrets-management.md); [Platform Engineering](../21-platform-engineering/README.md): [Self-service and tenancy](../21-platform-engineering/04-self-service-and-tenancy.md) | [Review AWS Architecture Read-Only](../labs/07-aws-architecture-review/README.md), [Test identity and application boundaries](../20-security/lab-security-boundaries.md); no direct IAM-at-scale incident drill; [Recoverable AWS Foundation with Terraform](../projects/04-aws-terraform-foundation/README.md) | G1, G3, G4; G2 remains open until an authorized IAM failure drill exists |
...
7. **Close AWS-specific gaps:** perform the bounded product exercises in the
   gap register in an authorized sandbox. Keep costs, identity, region, cleanup,
...
| Gap | Current coverage and honest status | Evidence needed to close |
|---|---|---|
| AWS-native CI/CD and artifacts | Delivery principles are strong; hands-on CodePipeline, CodeBuild, CodeDeploy, CodeArtifact, ECR, EC2 Image Builder, deployment agents, and AWS-managed pipeline secrets are **missing**. | In a sandbox, build once, test, publish by immutable identity, promote, fail a gate, diagnose a deployment, roll back, and preserve an audit trail. |
| CloudFormation-family IaC | Terraform graph, state, modules, tests, and drift are strong transferable coverage. CloudFormation, StackSets, SAM, CDK, modules, and Service Catalog practice is **missing**; Terraform use is not equivalent. | Deploy and update a reusable template, test replacement and rollback, detect drift, and exercise a bounded multi-account or simulated StackSet review. |
| Account vending and governance | IAM and governance models exist. Organizations, Control Tower, SCP policy evaluation, delegated administration, and account onboarding are **thin or missing**. | In an authorized organization sandbox, produce an account baseline, role path, SCP tests, centralized evidence, a denied-action diagnosis, and cleanup or teardown proof. |
| Fleet and configuration automation | Reconciliation and drift concepts are strong. Systems Manager Inventory, Patch Manager, State Manager, Automation, OpsCenter, AppConfig, AWS Config remediation, and OpsWorks are **missing**. | Manage a small disposable fleet or faithful sandbox fixture, introduce drift or a failed patch, prove bounded remediation and rollback, and account for partial failure. |
| AWS deployment targets | Container and Kubernetes mechanics are strong. Product-specific EC2, ECS, EKS, Lambda, API Gateway, Fargate, EFS, EBS, and S3 deployment choices are **thin**; serverless deployment practice is **missing**. | Deploy the same small workload to two relevant target types, compare mutable and immutable release paths, induce one failed release on each, and prove recovery. |
| Multi-Region resilience and AWS Backup | Availability, RTO, RPO, and recovery reasoning are covered. Service-specific RDS or Aurora, DynamoDB, Route 53, CloudFront, S3, and AWS Backup failover is **thin or missing**. Project 04 explicitly excludes deployed multi-Region failover. | Create a cost-bounded recovery design and test where authorized; capture measured RTO/RPO, stale or lost data bounds, DNS and client behavior, failback, and cleanup. |
| CloudWatch and AWS telemetry pipeline | Telemetry semantics and diagnosis are strong. CloudWatch agents, metric filters and streams, Logs Insights, X-Ray, Kinesis, Firehose, OpenSearch, Athena, and QuickSight configuration is **missing**. | Collect custom metrics and encrypted logs, set retention, query and correlate a fault, alarm on user impact, test missing telemetry, and calculate ingestion and retention cost. |
| Event-driven AWS operations | Queues, retries, idempotency, and control planes are strong. EventBridge patterns, AWS Health events, SNS, SQS, Kinesis, Lambda, and Step Functions integration is **thin or missing**. | Process replayed and duplicate events idempotently, route failures to durable handling, remediate one safe condition, reject one unsafe action, and show audit and recovery evidence. |
| AWS security automation and findings | Identity, cryptography, threat modeling, and incident response are strong. Security Hub, GuardDuty, Detective, Inspector, Macie, IAM Access Analyzer, Network Firewall, WAF, Shield, CloudHSM, and organization-scale aggregation are **missing**. | Enable a bounded subset in a sandbox, generate safe synthetic findings, route and triage them, test least privilege and encryption, suppress only with expiry, and document residual risk and cost. |
| Audit at organization scale | CloudTrail and configuration evidence appear conceptually and in project briefs. Organization trails, protected log archives, cross-account Config aggregation, anomalous-event alerting, and investigation workflows are **thin**. | Demonstrate tamper-resistant centralized audit, a query from event to principal and resource, an alert, a denied log-tampering attempt, retention policy, and break-glass review. |
